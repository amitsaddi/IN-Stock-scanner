"""
Database schema for Nifty 500 historical data storage
...
class NiftyDatabaseManager:
    """Manages SQLite database for Nifty 500 historical data"""
    ...
    def __init__(self, db_path: str = None):
        ...
        """
        if db_path is None:
            db_path = Path(__file__).parent.parent / "data" / "nifty500_historical.db"
    ...
    def create_stock_table(self, symbol: str):
        ...
        Args:
            symbol: Stock symbol (e.g., 'RELIANCE' without .NS)
        """
    ...
    def create_results_tracking_tables(self):
        ...
        Supports both Swing and BTST strategy types
        """
        try:
            ...
            self.cursor.execute("""
                CREATE TABLE IF NOT EXISTS backtest_results_history (
                    ...
                    strategy_type TEXT NOT NULL CHECK(strategy_type IN ('swing', 'btst')),
            ...
            self.conn.commit()
            logger.info("Created results tracking tables")
    ...
    def prune_old_data(self, symbol: str, before_date: str):
        ...
        try:
            ...
            if deleted > 0:
                logger.info(f"Pruned {deleted} old records from {table_name}")
...
if __name__ == "__main__":
    ...
    initialize_database(test_symbols, db_path="IN-testing/data/test.db")
    print("Done!")

    # Test connection and queries
    db = NiftyDatabaseManager(db_path="IN-testing/data/test.db")
    db.connect()
