...
BTST v2 Strategy Criteria (ENHANCED - Relaxed filters):
- Entry: 3:15 PM (market close)
- Exit: 9:15 AM next trading day (market open)
- Hold Period: Always 1 day
- Entry Criteria (RELAXED from v1):
  * Day gain: 1.5-4.0% (RELAXED from 2.0-3.5%)
  * Volume: >= 1.3x average (RELAXED from 1.5x)
  * High proximity: >= 85% (RELAXED from 90%)
  * RSI < 70 (NEW filter)
  * Positive MACD histogram (NEW filter)
  * ATR >= 1.0% (NEW filter)
- Min Score: 55 (LOWERED from 60)
- Max Candidates: 15 per day (INCREASED from 10)

Changes from v1:
1. Relaxed day gain range: 2.0-3.5% -> 1.5-4.0%
2. Relaxed volume: 1.5x -> 1.3x
3. Relaxed high proximity: 90% -> 85%
4. NEW RSI filter: < 70 (not overbought)
5. NEW MACD histogram filter
6. NEW ATR filter: >= 1.0% volatility
7. Lowered min score: 60 -> 55
8. More candidates: 10 -> 15
...
class BTSTBacktestV2:
    ...
    def __init__(self, db_path: str, verbose: bool = False):
        ...
        self.max_candidates = 15  # Lower than swing
    ...
    def check_btst_criteria(self, symbol: str, current_date: str, hist_df: pd.DataFrame) -> Tuple[bool, float, str]:
        """
        Check BTST v2 entry criteria at 3:15 PM (RELAXED)

        Criteria:
        1. Day Gain: 1.5-4.0% (30 points) - RELAXED
        2. Volume: >= 1.3x (20 points) - RELAXED
        3. High Proximity: >= 85% (15 points) - RELAXED
        4. RSI < 70 (10 points) - NEW
        5. MACD histogram > 0 (10 points) - NEW
        6. ATR >= 1.0% (10 points) - NEW

        Min score: 55 points (LOWERED from v1's 60)
        ...
        # 5. NEW: MACD histogram > 0 (10 points)
    ...
    def run_backtest(self, start_date: str, end_date: str) -> List[Trade]:
        ...
        """
        logger.info("=" * 80)
        logger.info(f"Starting BTST v2 Backtest (India): {start_date} to {end_date}")
        logger.info("=" * 80)
    ...
    def save_results(self, trades: List[Trade], metrics: PerformanceMetrics,
        ...
        # 1. Save trades CSV
        trades_file = output_path / f'btst_backtest_v2_trades_{timestamp}.csv'
        trades_df = pd.DataFrame([asdict(t) for t in trades])
        trades_df.to_csv(trades_file, index=False)
        logger.info(f"Saved detailed trades: {trades_file}")

        # 2. Save metrics JSON
        metrics_file = output_path / f'btst_backtest_v2_metrics_{timestamp}.json'
        with open(metrics_file, 'w') as f:
            json.dump(asdict(metrics), f, indent=2, cls=NumpyEncoder)
        logger.info(f"Saved performance metrics: {metrics_file}")

        # 3. Save report
        report_file = output_path / f'btst_backtest_v2_report_{timestamp}.txt'
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Backtest BTST v2 Strategy for Indian Market (Nifty 500)',
...
Examples:
  ...
  python btst_backtest_v2.py --start-date 2025-05-01 --end-date 2025-11-01 --verbose
        """
    )
