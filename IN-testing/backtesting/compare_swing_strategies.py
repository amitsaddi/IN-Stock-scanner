...
<head>
    ...
    <title>Swing Trading Strategy Comparison: v1 vs v2 (India)</title>
    <style>
        body {{
...
<body>
    ...
    <div class="footer">
        <p>Nifty 500 Swing Trading Strategy Analysis Framework</p>
...
"""
    ...
    def generate_all_reports(self, output_dir: str, formats: List[str] = ['html', 'txt', 'csv']):
        ...
        if 'html' in formats or 'all' in formats:
            html_file = output_path / f'swing_comparison_report_{timestamp}.html'
            self.generate_html_report(str(html_file))
            generated_files.append(html_file)

        if 'txt' in formats or 'all' in formats:
            txt_file = output_path / f'swing_comparison_summary_{timestamp}.txt'
            self.generate_txt_summary(str(txt_file))
            generated_files.append(txt_file)

        if 'csv' in formats or 'all' in formats:
            csv_file = output_path / f'swing_comparison_metrics_{timestamp}.csv'
...
def save_to_history_database(comparator: 'StrategyComparison', db_path: str, strategy_type: str = 'swing'):
    """
    ...
    try:
        ...
        """, (
            ...
            v1['avg_hold_days'], v1['median_hold_days'], v1.get('return_volatility', 0),
        ...
        """, (
            ...
            v2['avg_hold_days'], v2['median_hold_days'], v2.get('return_volatility', 0),
...
Examples:
  ...
  python compare_strategies.py \\
    ...
    try:
        ...
        save_to_history_database(comparator, db_path, strategy_type='swing')
