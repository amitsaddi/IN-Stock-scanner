"""
Comprehensive Backtesting Engine for v1 Swing Trading Strategy (Indian Market - Nifty 500)
...
- Fundamental: Market Cap >= ₹5000 Cr, D/E <= 0.5, ROE >= 15%
- Technical: RSI 40-60, Volume >= 1.2x, Price positioning vs MAs
...
- Sector Preferences: Defence, Capital Goods, Infrastructure, PSU Banks, Renewable Energy
...
# ============================================================================
# SECTOR WEIGHTS (V1 - INDIA NOV 2025)
# ============================================================================
...
class BacktestV1:
    """
    Comprehensive backtesting engine for v1 swing trading strategy (India)
    """
    ...
    def apply_fundamental_filters(self, metadata: pd.DataFrame) -> pd.DataFrame:
        ...
        Criteria:
        - Market Cap >= ₹5000 Cr
        - Debt-to-Equity <= 0.5 (stricter than AU's 1.0)
        - ROE >= 15% (stricter than AU's 10%)
        ...
        filtered = metadata[
            ...
            (metadata['debt_to_equity'] <= 0.5) &  # Stricter than AU
            (metadata['roe'] >= 15.0)  # Stricter than AU
        ].copy()
    ...
    def apply_technical_filters(self, df: pd.DataFrame) -> bool:
        ...
        Criteria:
        - RSI: 40-60 (narrower than AU's 35-65)
        - Volume ratio >= 1.2x
        - Price positioning vs MAs
        ...
        # RSI check (narrower range than AU)
    ...
    def calculate_technical_score(self, df: pd.DataFrame) -> float:
        """
        Calculate v1 technical score (20% of total)
        ...
        # RSI positioning (0-8 points) - optimal at 50
        rsi = latest.get('rsi', 50)
        # For RSI 40-60, score based on proximity to 50
        if 40 <= rsi <= 60:
            rsi_score = 8 * (1 - abs(rsi - 50) / 10)
            score += max(0, rsi_score)

        # MACD signals (0-7 points)
    ...
    def calculate_fundamental_score(self, market_cap: float, roe: float, debt_equity: float) -> float:
        """
        Calculate v1 fundamental score (20% of total)
        ...
        # ROE score (0-8 points) - normalize around 15-35%
        if roe >= 15:
            roe_normalized = min((roe - 15) / 20, 1.0)
            score += 8 * roe_normalized

        # Debt/Equity score (0-7 points) - lower is better, cap at 0.5
        de_score = (0.5 - min(debt_equity, 0.5)) / 0.5 * 7
        score += max(0, de_score)

        # Market cap score (0-5 points) - logarithmic scale
        # 5,000 Cr = baseline, 50,000+ Cr = max
        if market_cap >= 5000:
    ...
    def calculate_momentum_score(self, df: pd.DataFrame) -> float:
        """
        Calculate v1 momentum score (20% of total)
        ...
        if len(df) >= 5:
            ...
            if price_change_pct > 0:
                score += min(10, price_change_pct * 2)  # Cap at 10 points

        # MA trend strength (0-10 points)
        ema_20 = latest.get('ema_20', 0)
        ema_50 = latest.get('ema_50', 0)
        close = latest['close']
        ...
        if ema_20 > 0 and ema_50 > 0:
            ...
            if dist_20 > 0 and dist_50 > 0:
                score += min(10, (dist_20 + dist_50) * 2)

        return min(20, score)

    def calculate_volume_score(self, df: pd.DataFrame) -> float:
        """
        Calculate v1 volume score (20% of total)
        ...
        # 1.2x = 0 points, 2.5x+ = 20 points
        if volume_ratio >= 1.2:
            score = ((volume_ratio - 1.2) / 1.3) * 20
            return min(20, score)
    ...
    def calculate_total_score(self, df: pd.DataFrame, market_cap: float,
        ...
        base_score = tech_score + fund_score + momentum_score + volume_score + trend_score

        # Apply sector weight
        sector_weight = V1_SECTOR_WEIGHTS.get(sector.upper(), 1.0)
    ...
    def identify_entry_signal(self, df: pd.DataFrame) -> Optional[str]:
        ...
        Entry Signals:
        1. PULLBACK: RSI 40-50, price near/below 20 EMA
        ...
        # 1. Pullback (RSI in lower range, near support)
        if 40 <= rsi <= 50 and close <= ema_20 * 1.02:
            return 'pullback'
        ...
        # 4. Breakout (near 52W high with volume)
        if week_52_prox >= 90 and volume_ratio > 1.5:
            return 'breakout'

        # 5. Trend Follow (above all MAs, strong trend)
    ...
    def run_backtest(self, start_date: str, end_date: str) -> List[Trade]:
        ...
        """
        logger.info("=" * 80)
        logger.info(f"Starting v1 Strategy Backtest (India): {start_date} to {end_date}")
        logger.info("=" * 80)
    ...
    def save_results(self, trades: List[Trade], metrics: PerformanceMetrics,
        ...
        trades_file = output_path / f'swing_backtest_v1_trades_{timestamp}.csv'
        trades_df = pd.DataFrame([asdict(t) for t in trades])
        trades_df.to_csv(trades_file, index=False)
        logger.info(f"Saved detailed trades: {trades_file}")

        # 2. Save performance metrics JSON
        metrics_file = output_path / f'swing_backtest_v1_metrics_{timestamp}.json'
        ...
        report_file = output_path / f'swing_backtest_v1_report_{timestamp}.txt'
    ...
    def generate_report(self, metrics: PerformanceMetrics) -> str:
        ...
        report = []
        report.append("=" * 80)
        report.append("BACKTEST RESULTS - STRATEGY V1 (INDIA SWING TRADING - NIFTY 500)")
        report.append("=" * 80)
        report.append("")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Backtest v1 Swing Trading Strategy for Indian Market (Nifty 500)',
...
Examples:
  ...
  python swing_backtest_v1.py --start-date 2025-05-01 --end-date 2025-11-01 --verbose

  # Custom output directory
  python swing_backtest_v1.py --start-date 2025-05-01 --end-date 2025-11-01 --output-dir ./results
        """
    )
