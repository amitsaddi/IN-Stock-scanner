"""
Comprehensive Strategy Comparison Module for v1 vs v2 Backtesting Results

This module provides extensive analysis and reporting capabilities to compare
the performance of v1 (current) vs v2 (enhanced) swing trading strategies.
...
class StrategyComparison:
    ...
    def analyze_trade_distribution(self) -> Dict:
        ...
        distribution = {
            ...
            'hold_period': {
                'v1_avg': v1['avg_hold_days'],
                'v2_avg': v2['avg_hold_days'],
                'v1_median': v1['median_hold_days'],
                'v2_median': v2['median_hold_days'],
                'v1_std': np.std(v1_hold),
                'v2_std': np.std(v2_hold)
            },
            'streaks': {
...
<head>
    ...
    <title>Strategy Comparison: v1 vs v2</title>
    <style>
        body {{
...
<body>
    <div class="header">
        ...
        <p>v1 (Current) vs v2 (Enhanced) Swing Trading Strategy</p>
    ...
    <div class="footer">
        <p>ASX200 Swing Trading Strategy Analysis Framework</p>
...
"""
    ...
    <div class="section">
        ...
        <p>Comparison showing how v2's updated sector weights (2025 defensive positioning) affect performance.</p>
        <table>
            <thead>
                <tr>
                    <th>Sector</th>
...
"""
    ...
    def generate_txt_summary(self, output_path: str):
        ...
        summary.append("v1 (Current) vs v2 (Enhanced) Swing Trading Strategy")
        summary.append("=" * 80)
        summary.append("")
...
def save_to_history_database(
    ...
    """Save backtest results to history database for trend analysis"""
    import sqlite3
    ...
    try:
        ...
        cursor.execute("""
            ...
            (run_date, strategy_type, version, total_trades, winning_trades,
             losing_trades, win_rate, avg_return, total_return, sharpe_ratio,
             sortino_ratio, max_drawdown, avg_hold_days, median_hold_days,
             return_volatility, backtest_start_date, backtest_end_date,
             total_stocks_scanned, sector_performance, signal_distribution)
            VALUES (?, ?, 'v1', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            run_date, strategy_type,
            v1_metrics.get('total_trades', 0),
            v1_metrics.get('winning_trades', 0),
            v1_metrics.get('losing_trades', 0),
            v1_metrics.get('win_rate', 0.0),
            v1_metrics.get('avg_trade_pct', 0.0),
            v1_metrics.get('total_return_pct', 0.0),
            v1_metrics.get('sharpe_ratio'),
            v1_metrics.get('sortino_ratio'),
            v1_metrics.get('max_drawdown_pct'),
            v1_metrics.get('avg_hold_days'),
            v1_metrics.get('median_hold_days'),
            v1_metrics.get('return_volatility'),
            v1_metrics.get('start_date'),
            v1_metrics.get('end_date'),
            v1_metrics.get('total_stocks_scanned'),
            json.dumps(v1_metrics.get('sector_performance', {})),
            json.dumps(v1_metrics.get('entry_type_performance', {}))
        ))
        ...
        """, (
            run_date, strategy_type,
            v2_metrics.get('total_trades', 0),
            v2_metrics.get('winning_trades', 0),
            v2_metrics.get('losing_trades', 0),
            v2_metrics.get('win_rate', 0.0),
            v2_metrics.get('avg_trade_pct', 0.0),
            v2_metrics.get('total_return_pct', 0.0),
            v2_metrics.get('sharpe_ratio'),
            v2_metrics.get('sortino_ratio'),
            v2_metrics.get('max_drawdown_pct'),
            v2_metrics.get('avg_hold_days'),
            v2_metrics.get('median_hold_days'),
            v2_metrics.get('return_volatility'),
            v2_metrics.get('start_date'),
            v2_metrics.get('end_date'),
            v2_metrics.get('total_stocks_scanned'),
            json.dumps(v2_metrics.get('sector_performance', {})),
            json.dumps(v2_metrics.get('entry_type_performance', {}))
        ))

        # Map recommendation format
        recommendation_map = {
            'v1': 'use_v1',
            'v2': 'use_v2',
            'NEUTRAL': 'inconclusive'
        }
        db_recommendation = recommendation_map.get(recommendation, 'inconclusive')
        ...
        db_confidence = confidence_map.get(confidence, 'low')

        # Insert comparison results
        cursor.execute("""
            INSERT OR REPLACE INTO comparison_results_history
            (run_date, strategy_type, recommendation, confidence_level,
             criteria_met, total_criteria, returns_ttest_pvalue,
             winrate_chisquare_pvalue, delta_total_trades, delta_win_rate,
             delta_avg_return, delta_sharpe_ratio, delta_max_drawdown,
        ...
        """, (
            ...
            criteria_met,
            5,  # total_criteria
            statistical_tests.get('ttest_pvalue'),
            statistical_tests.get('chisquare_pvalue'),
            v2_metrics.get('total_trades', 0) - v1_metrics.get('total_trades', 0),
            v2_metrics.get('win_rate', 0.0) - v1_metrics.get('win_rate', 0.0),
            v2_metrics.get('avg_trade_pct', 0.0) - v1_metrics.get('avg_trade_pct', 0.0),
            (v2_metrics.get('sharpe_ratio') or 0) - (v1_metrics.get('sharpe_ratio') or 0),
            (v2_metrics.get('max_drawdown_pct') or 0) - (v1_metrics.get('max_drawdown_pct') or 0),
            output_files.get('html', ''),
            output_files.get('summary', ''),
        ...
        logger.info(f"Saved results to history database: {db_path}")

    except Exception as e:
        logger.error(f"Error saving to history database: {e}")
...
Examples:
  ...
  python compare_strategies.py \\
    ...
    parser.add_argument(
        '--output-dir',
        type=str,
        default='/Users/amitsaddi/Documents/git/IN-Stock-scanner/AU-testing/backtesting/comparison_reports',
    ...
    parser.add_argument(
        '--db-path',
        type=str,
        default='../data/asx200_historical.db',
        help='Path to database for storing results history (default: ../data/asx200_historical.db)'
    )
    ...
    try:
        ...
        for file_path in generated_files:
            ...
            if 'comparison_report' in file_name and file_name.endswith('.html'):
                output_file_map['html'] = file_name
            elif 'comparison_summary' in file_name and file_name.endswith('.txt'):
                output_file_map['summary'] = file_name
            elif 'comparison_metrics' in file_name and file_name.endswith('.csv'):
                output_file_map['csv'] = file_name

        comp = comparator.comparison

        # Calculate criteria met (count how many v2 improvement criteria were met)
        criteria_met = 0
        v1 = comparator.v1_metrics
        v2 = comparator.v2_metrics

        signal_increase_pct = ((v2['signals_per_week'] - v1['signals_per_week']) / v1['signals_per_week'] * 100) if v1['signals_per_week'] > 0 else 0
        if signal_increase_pct >= 30:
            criteria_met += 1
        if abs(v2['win_rate'] - v1['win_rate']) <= 5:
            criteria_met += 1
        avg_return_increase_pct = ((v2['avg_trade_pct'] - v1['avg_trade_pct']) / abs(v1['avg_trade_pct']) * 100) if v1['avg_trade_pct'] != 0 else 0
        if avg_return_increase_pct >= 10:
            criteria_met += 1
        if v2['max_drawdown_pct'] <= v1['max_drawdown_pct']:
            criteria_met += 1
        ...
        save_to_history_database(
            ...
            strategy_type='swing',
            v1_metrics=comparator.v1_metrics,
            v2_metrics=comparator.v2_metrics,
            recommendation=comp.recommended_strategy,
            confidence=comp.recommendation_confidence,
            criteria_met=criteria_met,
            output_files=output_file_map,
            statistical_tests={
                'ttest_pvalue': comp.returns_ttest_pvalue,
                'chisquare_pvalue': comp.win_rate_chi2_pvalue
            }
        )
